...
const login = async (req, res) => {
    try {
        ...
        if (!passwordMatch) {
            return res.status(401).json({ message: "Unauthorized access, Incorrect credentials" });
        }
        const token = await generateAccessToken(user._id);
        console.log(token)
        const userLogging = await User.findById(user._id)
...
const logout = async (req, res) => {
    try {
        ...
        return res.status(500).json({ message: `internal server error logout, something went wrong`, error: error.message })
    }
}
...
const followUser = async (req, res) => {
    try {
        ...
        const isFollowing = userfollowing.following.includes(userIdToFollow);

        if (isFollowing) {
            // If already following, unfollow
            userfollowing.following.pull(userIdToFollow);
            userToFollow.followers.pull(userfollowingid);
            await userfollowing.save();
            await userToFollow.save();
            return res.status(200).json({ message: "User unfollowed successfully", userfollowedBy: userfollowing, userfollowed: userToFollow });
        } else {
...
export { userSignup, login, logout, getCurrentUser, updateUserProfile, forgotpassword, resetPassword, resetpasswordui, followUser, followers, getAllUsers, searchUsers, deleteAccount, searchUserByid }